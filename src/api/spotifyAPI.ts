...
export const musicTokenAPI = {
  getToken() {
    return axios("https://accounts.spotify.com/api/token", {
      ...
      .then((token) => localStorage.setItem("spotifyToken", token));
  },
};
...
export const musicAPI = {
  ...
  search(query: string, type: string) {
    return $authHost
      .get(`https://api.spotify.com/v1/search?query=${query}&type=${type}`)
      .then((res) => res.data);
  },
  getArtistData(artistId: string) {
    return $authHost
      .get<ArtistDataType>(`https://api.spotify.com/v1/artists/${artistId}`)
      .then((res) => res.data);
  },
